...
parser.add_argument(
    "-t", "--textmul", type=float, default=DEFAULT_TEXTMUL, help="multiplier for text"
)
parser.add_argument(
    "-n",
    "--ntextmul",
    type=float,
    default=DEFAULT_NTEXTMUL,
    help="multiplier for non-text",
)
...
def main():
    ...
    semantle(
        args.filename, args.name, args.textmul, args.ntextmul, args.savefile, args.debug
    )
...
def semantle(
    file_name,
    cardname,
    textmul=DEFAULT_TEXTMUL,
    ntextmul=DEFAULT_NTEXTMUL,
    savefile=False,
    debug=False,
):
    #    print(cardname)
    ...
    distances_text = pandas.DataFrame(
        sklearn.metrics.pairwise_distances(
            only_text.values * textmul,
            only_text.loc[cardname].values.reshape(1, -1) * textmul,
            n_jobs=10,
        )
    )
    cards_no_text = cards.drop(columns=text_list)
    distances_no_text = pandas.DataFrame(
        sklearn.metrics.pairwise_distances(
            cards_no_text.values * ntextmul,
            cards_no_text.loc[cardname].values.reshape(1, -1) * ntextmul,
            n_jobs=10,
        )
    )
    ...
    distances = pandas.DataFrame(
        (distances_no_text.iloc[:, 0] ** 2 + distances_text.iloc[:, 0] ** 2).pow(1 / 2)
    )
