...
parser.add_argument(
    "-v", "--vector", action="store_true", help="convert cards to vector"
)
parser.add_argument(
    "-n", "--norm", action="store_true", help="run weights through vector"
)
parser.add_argument(
    "-y", "--type", type=float, default=TYPE_WEIGHT, help="weight for type embeddings"
)
parser.add_argument(
    "-x", "--text", type=float, default=TEXT_WEIGHT, help="weight for text embeddings"
)
parser.add_argument(
    "-m", "--mana", type=float, default=MANA_WEIGHT, help="weight for mana embeddings"
)
parser.add_argument(
    "-r",
    "--rarity",
    type=float,
    default=RARITY_WEIGHT,
    help="weight for rarity embedding",
)
parser.add_argument(
    "-p", "--power", type=float, default=POWER_WEIGHT, help="weight for power embedding"
)
parser.add_argument(
    "-t",
    "--toughness",
    type=float,
    default=POWER_WEIGHT,
    help="weight for toughness embedding",
)
parser.add_argument(
    "-c", "--cmc", type=float, default=CMC_WEIGHT, help="weight for cmc embedding"
)
...
def main():
    ...
    if args.vector:
        ...
        pandas.DataFrame(card_vecs).set_axis(colnames, axis=1).to_csv(
            save_name, index=0
        )
    ...
    if args.norm:
        ...
        if list(cards.columns) != colnames:  # checks that header matches expected
            ...
            cards = weight_cards(
                cards, args
            )  # weight_cards does all weights and normalizations
...
def weight_section(section: pandas.DataFrame, width):
    if width > 1:
        for column in section:
            ...
            if max == 0:
                continue
            section[column] = section[column].apply(
                lambda x: adjusted_values(x, width, max)
            )
...
def card2vec(cards_data):
    ...
    type_vecs = type_vecs.apply(
        lambda row: type2vec(str(row)), axis="columns", result_type="expand"
    )
    type_vecs.columns = [
        "Creature",
        "Instant",
    ...
    mana_vecs = mana_vecs.apply(
        lambda row: mana2vec(row.values[0]), axis="columns", result_type="expand"
    )
    mana_vecs.columns = [
        "W",
        "U",
    ...
    rarity_vec = rarity_vec.apply(
        lambda row: rarity2vec(row.values[0]), axis="columns", result_type="expand"
    )
    rarity_vec.columns = ["Rarity"]

    power_vec = cards_data.loc[:, "power"].to_frame()
    power_vec = power_vec.apply(
        lambda row: (
            0
            if isinstance(row.values[0], str) or not (row.values[0].is_integer())
            else row.values[0]
        ),
        axis="columns",
        result_type="expand",
    ...
    toughness_vec = toughness_vec.apply(
        lambda row: (
            0
            if isinstance(row.values[0], str) or not (row.values[0].is_integer())
            else row.values[0]
    ...
    text_vecs = text2vec(
        cards_data.loc[:, "name"].map(str).values
        + " "
        + cards_data["type_line"].map(str).values
        + " "
        + cards_data["oracle_text"].map(str).values
    )
...
def text2vec(text):
    ...
    model = Doc2Vec(vector_size=30, min_count=5, epochs=60)
    model.build_vocab(tagged_data)
    model.train(tagged_data, total_examples=model.corpus_count, epochs=model.epochs)

    # get the document vectors
    document_vectors = [model.infer_vector(word_tokenize(doc.lower())) for doc in text]
